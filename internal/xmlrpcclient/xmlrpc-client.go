...
func (r *XMLRPCClient) postInetHTTP(method string, url string, data interface{}, processBody func(io.ReadCloser, error)) {
	...
	if err != nil {
		processBody(emptyReader, fmt.Errorf("fail to send http request to supervisord: %w", err))
		return
	}
...
func (r *XMLRPCClient) postUnixHTTP(method string, path string, data interface{}, processBody func(io.ReadCloser, error)) {
	...
	if err != nil {
		processBody(emptyReader, fmt.Errorf("fail to connect unix socket path %s: %w", r.serverurl, err))
		return
	}
	defer conn.Close()
	...
	if err != nil {
		processBody(emptyReader, fmt.Errorf("fail to create http request: %w", err))
		return
	}
	err = req.Write(conn)
	if err != nil {
	...
	if err != nil {
		processBody(emptyReader, fmt.Errorf("fail to read response: %w", err))
		return
	}
	r.processResponse(resp, processBody)
