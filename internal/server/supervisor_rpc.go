...
//
// Per-process failures are surfaced through the per-result Status /
// Description fields so callers can tell which program's clear failed; the
// loop continues for the rest. The RPC itself returns nil unless the
// underlying iteration cannot run.
func (s *Supervisor) ClearAllProcessLogs(r *http.Request, args *struct{}, reply *struct{ RPCTaskResults []RPCTaskResult }) error {
	s.procMgr.ForEachProcess(func(proc *process.Process) {
		errOut := proc.StdoutLog.ClearAllLogFile()
		errErr := proc.StderrLog.ClearAllLogFile()
		procInfo := getProcessInfo(proc)
		result := RPCTaskResult{
			Name:        procInfo.Name,
		...
		switch {
		case errOut != nil && errErr != nil:
			result.Status = faults.Failed
			result.Description = fmt.Sprintf("stdout: %v; stderr: %v", errOut, errErr)
		case errOut != nil:
			result.Status = faults.Failed
			result.Description = fmt.Sprintf("stdout: %v", errOut)
		case errErr != nil:
			result.Status = faults.Failed
			result.Description = fmt.Sprintf("stderr: %v", errErr)
		}
		reply.RPCTaskResults = append(reply.RPCTaskResults, result)
	})
	return nil
}
