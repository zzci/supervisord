...
func newProgramEntry(name, dependsOn string) *Entry {
	e := NewEntry(".")
	e.Group = "group:default"
	e.Name = "program:" + name
	if dependsOn != "" {
		e.keyValues["depends_on"] = dependsOn
	}
	return e
}

func TestValidateDependsOnAcyclic(t *testing.T) {
	entries := []*Entry{
		newProgramEntry("a", ""),
		newProgramEntry("b", "a"),
		newProgramEntry("c", "a, b"),
	}
	if err := ValidateProgramDependsOn(entries); err != nil {
		t.Fatalf("expected no error on acyclic graph, got: %v", err)
	}
}
...
func TestValidateDependsOnCycle(t *testing.T) {
	...
	err := ValidateProgramDependsOn(entries)
	if err == nil {
		t.Fatal("expected cycle error, got nil")
	}
}

func TestValidateDependsOnDanglingRef(t *testing.T) {
	entries := []*Entry{
		newProgramEntry("a", "ghost"),
	}
	err := ValidateProgramDependsOn(entries)
	if err == nil {
