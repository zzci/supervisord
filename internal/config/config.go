...
//
// Each call resets the in-memory entry map so that keys removed from
// supervisor.conf since the previous Load() actually disappear; reusing
// the existing map would let stale keys survive a reload.
func (c *Config) Load() ([]string, error) {
	...
	loaded := c.parse(myini)
	if err := ValidateProgramDependsOn(c.GetPrograms()); err != nil {
		return loaded, err
	}
	return loaded, nil
}

func (c *Config) getIncludeFiles(cfg *Ini) []string {
...
func (c *Config) parse(cfg *Ini) []string {
...
func (c *Config) setProgramDefaultParams(cfg *Ini) {
...
func parseEnvFiles(s string) *map[string]string {
	...
	for _, envFilePath := range strings.Split(s, ",") {
		...
		// #nosec G304 -- envFiles paths come from supervisor.conf, by design
...
func (c *Entry) parse(section *Section) {
	c.Name = section.Name
	// Reset key/value map so reloaded sections see the new file authoritatively
	// and do not retain keys that were removed from the file.
	c.keyValues = make(map[string]string)
	for _, key := range section.Keys() {
		c.keyValues[key.Name()] = strings.TrimSpace(key.ValueWithDefault(""))
	}
}

func (c *Config) parseGroup(cfg *Ini) {
...
func (c *Config) isProgramOrEventListener(section *Section) (bool, string) {
...
func (c *Config) parseProgram(cfg *Ini) []string {
