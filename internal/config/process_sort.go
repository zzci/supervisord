...
// ValidateDependsOn checks the depends_on graph for cycles and dangling
// references. Returns the first problem found; nil if the graph is OK.
func (p *ProcessSorter) ValidateDependsOn(programConfigs []*Entry) error {
	p.initDepends(programConfigs)

	known := make(map[string]struct{}, len(programConfigs))
	for _, cfg := range programConfigs {
		if cfg.IsProgram() {
			known[cfg.GetProgramName()] = struct{}{}
		}
	}
	for prog, deps := range p.dependsOnGraph {
		for _, dep := range deps {
			if _, ok := known[dep]; !ok {
				return fmt.Errorf("program %q depends_on unknown program %q", prog, dep)
			}
		}
	}
	...
	color := make(map[string]int, len(p.dependsOnGraph))
	stack := make([]string, 0)
	...
	visit = func(node string) error {
		...
		for _, dep := range p.dependsOnGraph[node] {
	...
	for node := range p.dependsOnGraph {
...
// ValidateProgramDependsOn is a convenience wrapper that constructs a fresh
// sorter and validates the depends_on graph in one call.
func ValidateProgramDependsOn(programConfigs []*Entry) error {
	return NewProcessSorter().ValidateDependsOn(programConfigs)
}
