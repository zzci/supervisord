...
func TestEventListenerManagerConcurrent(t *testing.T) {
	mgr := NewEventListenerManager()

	// Spin up listener registrations and emits concurrently. The intent
	// is to trip the race detector if the manager's maps are accessed
		...
		go func(id int) {
			...
			for j := 0; j < ops; j++ {
				listener := NewEventListener(
					"pool-"+name, "stdin-"+name,
					noopReader{},
					noopWriter{}, 16,
				)
				mgr.registerEventListener(name, []string{"PROCESS_STATE"}, listener)
				mgr.EmitEvent(CreateProcessStartingEvent("p", "g", "STOPPED", 0))
				mgr.unregisterEventListener(name)
			}
		}(i)
	}
	for i := 0; i < goroutines; i++ {
