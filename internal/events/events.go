...
//
// Both maps are guarded by mu. EmitEvent takes a read lock just long enough
// to snapshot the listener set for the event type, then releases the lock
// before dispatching, so a listener handler that re-enters the manager
// (e.g. reload-time register/unregister) does not deadlock or race.
...
func (em *EventListenerManager) EmitEvent(event Event) {
	// Snapshot the listener set under a read lock so concurrent
	// register/unregister cannot mutate the map while we iterate, and so
	// listener handlers (which may re-enter to register or emit) do not
	// deadlock against a held lock.
	em.mu.RLock()
	listeners := em.eventListeners[event.GetType()]
	if len(listeners) == 0 {
		em.mu.RUnlock()
		return
	}
